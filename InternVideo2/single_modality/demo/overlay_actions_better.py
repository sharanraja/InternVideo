...
# Calculate the height of the text background and the vertical offset for the text
text_background_height = 60  # Height for the text background
background_y_offset = int(0.10 * frame_height)  # 10% from the top of the frame
...
while True:
    ...
    if text:
        ...
        overlay = frame.copy()
        text_background = np.full((text_background_height, frame_width, 3), background_color, dtype=np.uint8)

        # Calculate text size
        (text_width, text_height), baseline = cv2.getTextSize(text, font, font_scale, font_thickness)
        text_x = int((frame_width - text_width) / 2)
        text_y = int((text_background_height + text_height) / 2)

        # Draw the text background
        text_background = cv2.rectangle(text_background, (0, 0), (frame_width, text_background_height), background_color, -1)
        ...
        # Overlay the text background on the top of the frame, 10% from the top
        frame[background_y_offset:background_y_offset + text_background_height, :] = text_background
